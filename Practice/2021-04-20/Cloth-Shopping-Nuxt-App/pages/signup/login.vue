<template>
  <v-container>
    <v-layout>
      <!-- <v-spacer></v-spacer> -->
      <v-flex xs5 offset-xs4>
        <v-card>
          <v-card-title
            ><h1 class="display-1 ma-auto">Log In</h1>
            <!-- <br /> -->
          </v-card-title>
          <v-card-text>
            <v-flex>
              <h4 class="subtitle-1 mx-9 my-3">
                Sign in to access your Orders, Wishlist and Recommendations
              </h4>
              <h2 class="my-6 black--text">Continue With</h2>
              <v-row align="center" class="my-6" justify="space-around">
                <v-btn tile class="elevation-0">
                  <v-img
                    src="https://images.koovs.com/uploads/koovs/koovsglogo.jpg"
                    alt="google"
                    width="20px"
                  />&nbsp; Google
                </v-btn>
                <v-btn tile class="elevation-0">
                  <v-img
                    src="data:image/png;base64,"
                    alt="facebook"
                    width="35px"
                  />&nbsp; Facebook
                </v-btn>
              </v-row>
              <h2 class="mt-8 mb-2 black--text">Using Email</h2>
              <v-text-field
                v-model="email"
                placeholder="Email"
                type="email"
                dense
                outlined
              ></v-text-field>
              <v-text-field
                v-model="password"
                placeholder="Password"
                type="password"
                dense
                outlined
              ></v-text-field>
              <v-btn class="black" width="200px" x-large dark @click="login"
                >Login</v-btn
              >
              <v-layout class="mt-4">
                <nuxt-link to="/signup/reset">Reset Password</nuxt-link>
                <v-spacer></v-spacer>
                <span class="font-weight-bold">New to koovs?</span>
                <nuxt-link to="/signup/register">Create Account</nuxt-link>
              </v-layout>
            </v-flex>
          </v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
  </v-container>
</template>

<script>
export default {
  data() {
    return {
      email: '',
      password: '',
    }
  },
  methods: {
    login() {
      this.$axios
        .post(
          `https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${process.env.AppId}`,
          {
            email: this.email,
            password: this.password,
            returnSecureToken: true,
          }
        )
        .then((res) => {
          this.$cookies.set('token', res.data.idToken)
          this.$store.commit('setLoggedIn', true)
          this.$router.push('/checkout/wishlist')
        })
        .catch((err) => alert(err.response.data.error.message))
    },
  },
}
</script>

<style scoped>
a {
  text-decoration: none;
  color: black;
}
</style>
